<?php
  ...
  <?php 
    ...
    $_SESSION["recher_affectation"]=$recher_affectation;
	 ...
	 $requete="SELECT * FROM fiche WHERE id_fiche!=''AND etat_fiche=0 AND decaisse=0 AND sauvegarder=0 AND approuve=0 AND affectation_id=1 AND conforme=0 ";
  ...
  if($last_page > 1){
        ...
        if($page > 1)
            $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(1);'>&laquo; Debut</button>";
        else
    ...
    if ($page > 1)
            $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(".($prev).");'>&laquo; Precedent&nbsp;&nbsp;</button>";
        else
        ...
        {   
            for ($counter = 1; $counter <= $last_page; $counter++)
            {
                if ($counter == $page)
                    ...
                    $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(".($counter).");'>$counter</button>";     
        ...
        elseif($last_page > 5 + ($adjacents * 2))
        {
            if($page < 1 + ($adjacents * 2))
            {
                ...
                $pagination.= "...";
                $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(".($second_last).");'> $second_last</button>";
                $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(".($last_page).");'>$last_page</button>";   
           
           }
           elseif($last_page - ($adjacents * 2) > $page && $page > ($adjacents * 2))
           {
               $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(1);'>1</button>";
               $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(2);'>2</button>";
               $pagination.= "...";
        ...
        if($page < $counter - 1)
            $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(".($next).");'>Suivant &raquo;</button>";
        else
    ...
    if($page < $last_page)
            $pagination.= "<button tabindex='0' role='button' href='javascript:void(0);' onClick='change_page_verif_conforme(".($last_page).");'>Fin &raquo;</button>";
        else
    ...
    $_SESSION['recher_affectation']=$recher_affectation;
	 ...
	 $req="SELECT * FROM fiche WHERE id_fiche!='' AND approuve=0 AND etat_fiche=0 AND sauvegarder=0 AND affectation_id=1 AND conforme=0 ";
...
$HTML.='
    <a href="exportation/pdf/pdf_liste_verif_conforme.php" target="_blank" style="font-size:20px; font-weight:600; color: #da0909" title="Générer le fichier pdf">
    ...
    foreach($records as $row) {
        ...
        <div class="card border shadow-none">
            ...
            <div class="row">
                ...
                <div class="d-flex align-items-start">
                    <div class="flex-shrink-0 avatar rounded-circle me-4">
                        <img style="height:100px; width:100px;" src="../../img_demande/'; if($row['photo_beneficiaire']!=''){$HTML.= $row['photo_beneficiaire'];}else{ $HTML.= 'default_picture.png';} $HTML.='" alt="" class="img-fluid rounded-circle">
                    </div>
         ...
         //   if($_SESSION['is_valid']==1){
                $HTML.= '
                    <a href="src/conforme_fiche.php?num_fiche_conforme='.$row['num_fiche'].'" class="btn btn-success"><button type="button" class="btn btn-outline-light" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Profile"><i class="uil uil-check"></i></button></a>
                    <a href="detail_desconforme.php?num_fiche_desconforme='.$row['num_fiche'].'" class="btn btn-danger"><button type="button" class="btn btn-outline-light" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Projects"><i class="uil uil-trash"></i></button></a>
                    <a target="_blank" href="modifier_fiche.php?num_fiche='.$row['num_fiche'].'" class="btn btn-warning"><button type="button" class="btn btn-outline-light" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Profile"><i class="uil uil-edit"></i></button></a>
                ';
            //    }
