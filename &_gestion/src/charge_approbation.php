<?php
  ...
  <?php 
    ...
    $_SESSION["recher_affectation"]=$recher_affectation;
		 ...
		 $requete="SELECT * FROM fiche WHERE id_fiche!='' AND approuve=0 AND etat_fiche=0 AND sauvegarder=0 ";
     ...
     if($_SESSION['resp_logistique']==1){
        ...
        $requete.=" AND ((affectation_id=18 AND serv_bureau_fidest_id=8) OR (affectation_id=19 AND serv_bureau_banamur_id=8)) ";
     ...
     if($_SESSION['resp_bureau']==1){
        ...
        $requete.=" AND (affectation_id=18 OR affectation_id=19)  ";
...
/*
    ...
    if ($_SESSION['resp_technique'] == 1 || $_SESSION['verif_conforme'] == 1) {
        ...
        if ($_SESSION['resp_technique'] == 1 || $_SESSION['verif_conforme'] == 1) {
        if (isset($_SESSION['resp_ch41']) && $_SESSION['resp_ch41'] == 1) {
            $requete.= " AND affectation_id=1 AND conforme=1 AND chantier_id=44 ";
        } else {
            $requete.= " AND affectation_id=1 AND conforme=1 AND chantier_id!=44 ";
        }
    }
...
$HTML.='
    <a href="exportation/pdf/pdf_liste_approbation.php" target="_blank" style="font-size:20px; font-weight:600; color: #da0909" title="Générer le fichier pdf">
    ...
    foreach($records as $row) {
        ...
        <div class="card border shadow-none">
         ...
         //   if($_SESSION['is_valid']==1){
                $HTML.= '
                    <a href="src/approuver_fiche.php?num_fiche_approuve='.$row['num_fiche'].'" class="btn btn-success"><button type="button" class="btn btn-outline-light" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Profile"><i class="uil uil-check"></i></button></a>
                    <a href="detail_desaprobation.php?num_fiche_desapprouve='.$row['num_fiche'].'" class="btn btn-danger"><button type="button" class="btn btn-outline-light" data-bs-toggle="tooltip" data-bs-placement="top" aria-label="Projects"><i class="uil uil-trash"></i></button></a>
