...
$montant_decaisser = $irep1['montant_fiche'];
...
// if ($smdp > 0 && $_SESSION['part_dec'] != 'yes') {
//   $montant_decaisser = $irep1['montant_fiche'] - $smdp;
// }
...
// if ($_SESSION['part_dec'] == 'yes') {
//   $montant_decaisser = $_SESSION['montant_encaisse'];
// }
...
if ($montant_restant == 0) {
  ...
  $req = $con->prepare('UPDATE fiche SET decaisse=1, sauvegarder=0, secur_decaisse="dgfidest", secur_valid="dgfidest", etat_fiche=1 WHERE num_fiche=:A');
...
try {
  ...
  try {
    ...
    $stmt = $con->prepare("SELECT *  FROM fiche WHERE num_fiche=:num_fiche");
    ...
    //Envoi notif whatsapp
     ...
     $whatsappNumber = "+225" . $row['tel_beneficiaire_fiche'];
    $num_fiche = $row['num_fiche'];

    $whatsapp->sendConfirmationDecaissement($whatsappNumber, $row['beficiaire_fiche'], $row['num_fiche'], $row['precision_fiche']);
...
<body>
  ...
  <div class="container">
    ...
    <p>Vous pouvez maintenant décaisser la somme de <?php echo number_format($montant_decaisser, 0, ',', ' '); ?> FCFA.</p>
