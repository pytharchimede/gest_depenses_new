...

{
...

    {
        ...
        addImageSafely($this, '../../../img/logo_veritas.jpg', 10, 275, 30, 0, '', '', 'Logo manquant');
        ...
        addImageSafely($this, '../../../img/logo_veritas.jpg', 172, 275, 30, 0, '', '', 'Logo manquant');
...
// Fonction globale pour ajouter une image de manière sécurisée
function addImageSafely($pdf, $file, $x, $y, $w, $h = 0, $type = '', $link = '', $errorMsg = '')
{
    // Vérifier si le fichier existe et n'est pas vide
    if (!file_exists($file) || filesize($file) == 0) {
        addErrorMessageToPDF($pdf, $x, $y, $w, $h, $errorMsg ?: 'Image manquante');
        return false;
    }

    // Vérifier la taille du fichier
    if (filesize($file) < 10) {
        addErrorMessageToPDF($pdf, $x, $y, $w, $h, $errorMsg ?: 'Fichier corrompu');
        return false;
    }

    // Vérifier le type d'image en lisant l'en-tête du fichier
    $imageInfo = @getimagesize($file);
    if ($imageInfo === false) {
        addErrorMessageToPDF($pdf, $x, $y, $w, $h, $errorMsg ?: 'Format non supporté');
        return false;
    }

    // Types d'images supportés par FPDF
    $supportedTypes = [IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_GIF];
    if (!in_array($imageInfo[2], $supportedTypes)) {
        addErrorMessageToPDF($pdf, $x, $y, $w, $h, $errorMsg ?: 'Type non supporté');
        return false;
    }

    // Utiliser un gestionnaire d'erreurs personnalisé pour intercepter les erreurs FPDF
    $previousErrorHandler = set_error_handler(function ($severity, $message, $file, $line) {
        // Ne pas afficher l'erreur, juste la capturer
        return true;
    });
    ...
    try {
        ...
        addErrorMessageToPDF($pdf, $x, $y, $w, $h, $errorMsg ?: 'Erreur FPDF');
        return false;
    } catch (Error $e) {
        // Pour les erreurs fatales PHP 7+
        set_error_handler($previousErrorHandler);

        addErrorMessageToPDF($pdf, $x, $y, $w, $h, $errorMsg ?: 'Erreur fatale');
        return false;
    }
}

// Fonction pour afficher un message d'erreur à la place d'une image
function addErrorMessageToPDF($pdf, $x, $y, $w, $h, $message)
{
    // Sauvegarder la position actuelle
    $currentX = $pdf->GetX();
    $currentY = $pdf->GetY();

    // Dessiner un rectangle pour remplacer l'image
    $pdf->SetDrawColor(200, 200, 200);
    $pdf->SetFillColor(245, 245, 245);
    $pdf->Rect($x, $y, $w, $h ?: 10, 'DF');

    // Ajouter le texte d'erreur
    $pdf->SetXY($x + 2, $y + ($h ?: 10) / 2 - 2);
    $pdf->SetFont('Arial', 'I', 6);
    $pdf->SetTextColor(150, 150, 150);
    $pdf->Cell($w - 4, 4, utf8_decode($message), 0, 0, 'C');
...
addImageSafely($pdf, "../../../img/entete_fiche.jpg", 5, 10, 200, 0, '', '', 'En-tête manquant');
...
if ($info_fiche0['photo_beneficiaire'] != '') {
    $photo = $info_fiche0['photo_beneficiaire'];
} else {
    $photo = 'default_picture.jpg';
}
...
addImageSafely($pdf, "../../../../img_demande/" . $photo, 12, 48, 25, 0, '', '', 'Photo manquante');
...
addImageSafely($pdf, "qr_code_" . $_SESSION['num_fiche'] . ".png", 170, 52, 30, 0, '', '', 'QR Code indisponible');
...
if ($info_fiche0['etat_fiche'] != 0) {
    ...
    addImageSafely($pdf, "../../../img/signature_small.jpg", 150, 235, 50, 0, '', '', 'Signature manquante');
}
...
if ($info_fiche0['signature_beneficiaire'] != '' && $montant_dispo == 0 && ($montant_fiche == $montant_den)) {
    ...
    addImageSafely($pdf, "../../../images/cachet_paye.jpg", 120, 170, 75, 0, '', '', 'Cachet manquant');
}
...
if ($info_fiche0['approuve'] == 1) {
    ...
    addImageSafely($pdf, "../../../img_sign/" . $info_fiche0['signature_util'], 110, 235, 50, 0, '', '', 'Signature manquante');
}
...
if ($info_fiche0['cni_beneficiaire'] != '') {
    $img_cni = $info_fiche0['cni_beneficiaire'];
} else {
    $img_cni = 'cni_benef.jpg';
}

//$sign_y=13.5*floatval($nbre_serv);

addImageSafely($pdf, "../../../img/" . $img_cni, 38, 48, 15, 0, '', '', 'CNI manquante');



if ($info_fiche0['signature_beneficiaire'] != '') {
    $img_signature_beneficiaire = $info_fiche0['signature_beneficiaire'];
} else {
    $img_signature_beneficiaire = 'signature.jpg';
}

//$sign_y=13.5*floatval($nbre_serv);

addImageSafely($pdf, "../../signature/" . $img_signature_beneficiaire, 25, 238, 30, 0, '', '', 'Signature manquante');
