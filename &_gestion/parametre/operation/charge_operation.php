...
$i=1;
    ...
    foreach($records as $row) {
        ...
        //Calcul cout prev
        $cout_prev=0;
        $cpr=$con->prepare('SELECT * FROM designation LEFT JOIN operation ON designation.operation_id_designation=operation.id_operation WHERE operation_id_designation=:A');
        $cpr->execute(array('A'=>$row['id_operation']));
        while($icpr=$cpr->fetch()){
            if (is_numeric($icpr['montant_debourse']) && is_numeric($cout_prev)) {
                $cout_prev += $icpr['montant_debourse'];
            }
            else
            {
                $cout_prev='Erreur !!!';
        ...
        $cout_reel=0;
        /*
        $cre=$con->prepare('SELECT * FROM fiche LEFT JOIN chantier ON chantier.id_chantier=fiche.chantier_id LEFT JOIN operation ON operation.chantier_id_operation=chantier.id_chantier LEFT JOIN designation ON designation.operation_id_designation=operation.id_operation LEFT JOIN decaissement ON decaissement.num_fiche_decaissement=fiche.num_fiche WHERE operation_id_designation=:A  AND affectation_id=1 ');
        $cre->execute(array('A'=>$row['id_operation']));
        while($icre=$cre->fetch()){
            if (is_numeric($icre['montant']) && is_numeric($cout_reel)) {
                $cout_reel += $icre['montant'];
            }
            else
            {
                $cout_reel='Erreur !!!';
...
<tr>
            ...
            <td><span class="text-muted" style="color:#CC9900; font-weight:600; text-transform:uppercase">'.$cout_prev.'</span></td>
            <td><span class="text-muted" style="color:#CC9900; font-weight:600; text-transform:uppercase">'.$cout_reel.'</span></td>
            <td><span class="text-muted" style="color:#CC9900; font-weight:600; text-transform:uppercase"></span></td>
            <td><span class="text-muted" style="color:#CC9900; font-weight:600; text-transform:uppercase"></span></td>
            <td><span class="text-muted" style="color:#CC9900; font-weight:600; text-transform:uppercase"></span></td>
            <td><span class="text-muted" style="color:#CC9900; font-weight:600; text-transform:uppercase"></span></td>
