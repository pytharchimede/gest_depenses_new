...
$lib_operation=addslashes($liste[0]);
$chantier_id_operation=addslashes($liste[1]);


$cpt++;

$red=$con->prepare("SELECT * FROM operation WHERE lib_operation =:A AND chantier_id_operation=:B "); 
$red->execute(array('A' => $lib_operation, 'B'=>$chantier_id_operation ));
...
if ($red->rowCount())
{
  ...
  $lib_doublon='Doublon détecté sur la valeur <b>'.$lib_operation.'</b> pour le chantier N° <b>'.$chantier_id_operation.'</b>  Veuillez vérifier votre base de données';
...
else
{
  ...
  $req=$con->prepare("INSERT INTO operation(lib_operation, chantier_id_operation) VALUES (:A, :B)");
  $req->execute(array('A'=>$lib_operation, 'B'=>$chantier_id_operation));
}
...
$date=gmdate('Y-m-d H:i:s');
  ...
  $lib_trace="Ajout de l\'operation <b>".$lib_operation."</b> sur le chantier N° <b>".$chantier_id_operation."</b> ";
